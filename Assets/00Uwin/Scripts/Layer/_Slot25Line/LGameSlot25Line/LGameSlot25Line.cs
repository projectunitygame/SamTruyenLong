...
public class LGameSlot25Line : UILayer {
    ...
    public override void Close () {
        ...
        ClearUI ();
        UILayerController.Instance.GetLayer<LGameSlot25LineLobby> ().Close ();
    }
    #endregion
    ...
    public void ButtonLineClickListener () {
        if (accountSpin.FreeSpins > 0) {
            ...
            // });
            StopAnimLastResult ();
            if (cachedLines.Count < 25)
                cachedLines.Add (cachedLines[cachedLines.Count - 1] + 1);
            else
                cachedLines = new List<int> { 1 };
            _machine.InitLineSelected (cachedLines);
            _machine.ShowLineSelected (cachedLines, 30);
            SetLineSelected (cachedLines);
            Database.Instance.SaveLast25Line(cachedLines);
        }
    }
    ...
    public void ButtonBetClickListener () {
        ...
        if (this.roomBetId <= 3) {
            this.roomBetId++;
            ButtonChangeBetClickListener (this.roomBetId);
        } else {
            this.roomBetId = 1;
            ButtonChangeBetClickListener (this.roomBetId);
        }
    ...
    public void ButtonSieuTocClickListener () {
        ...
        _machine.SetSpeed (isSieuToc ? 5 : 4);
    }
    ...
    public void Init (SRSSlot25LineConfig config, Slot25lineSignalRServer server, SRSSlot25LineAccount account, AssetBundleSettingItem assetBundleConfig, int roomId, int mType) {
        ...
        SetNotifyFree ();
        cachedLines = Database.Instance.lastSelectLine25Slots;
        _machine.InitLineSelected (Database.Instance.lastSelectLine25Slots);
        _machine.ShowLineSelected (Database.Instance.lastSelectLine25Slots, 10);
        SetLineSelected (Database.Instance.lastSelectLine25Slots);
        ...
        if (this.accountSpin.GetCollectedPieces ().Count == 9) {
            ...
            miniGameMoneyDes.text = "(" + this.accountSpin.TotalGoldUsed / 100 + "x" + this.accountSpin.PricePoolMultiply + ")";
    ...
    #endregion
    IEnumerator ThongBaoTrung (string money) {
        tbText.text = money;
        tb.SetActive (true);
        yield return new WaitForSecondsRealtime (4);
        tb.SetActive (false);
        miniGameController.ReloadMiniGame (new List<int> (), System.DateTime.MinValue);
        miniGameTotalMoney.text = "0";
        miniGameMoneyDes.text = "(0x0)";
        _server.HubCallCleanMiniGameData (accountInfo.AccountID);
    }
    #region Slot Machine
    // quay spin
    IEnumerator WaitMachineFinish (SRSSlot25LineResultSpin result) {
        ...
        if (result.isReciedPieces) {
            if (result.GetCollectedPieces ().Count == 9) {
                ...
                miniGameMoneyDes.text = "(" + result.TotalGoldUsed / 100 + "x" + result.PricePoolMultiply + ")";
                StartCoroutine (ThongBaoTrung (result.GetPrizePool ().ToString ()));
                ...
                miniGameMoneyDes.text = "(" + result.TotalGoldUsed / 100 + "x" + result.PricePoolMultiply + ")";
                mg.SetActive (true);
                mg.GetComponent<SkeletonGraphic> ().AnimationState.SetAnimation (0, "jackpotsamtruyen", false).Complete +=
                    delegate (TrackEntry track) {
                        mg.SetActive (false);
                    };
