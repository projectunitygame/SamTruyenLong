...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        // hoang anh add
        protected bool allowSendIndex = false;
        ...
        public void Load<T>(List<T> datas, bool allowSendIndex)
        {
            this.allowSendIndex = allowSendIndex;

            this.datas = datas.Select(a => (object)a).ToList();
            totalCount = datas.Count;
            RefillCells(itemTypeStart);
        }

        public void Load<T>(List<T> datas)
        {
            StopMovement();
            //ClearCells();
            ...
            if (itemTypeStart > totalCount)
                itemTypeStart = totalCount - content.childCount;
        ...
        public void LoadToEnd<T>(List<T> datas)
        {
            this.datas = datas.Select(a => (object)a).ToList();
            totalCount = datas.Count;
        ...
        //        if (itemTypeStart > totalCount - 12)
        //            RefillCells(itemTypeStart);
        //    }
        //}
        ...
        private void SendMessageToNewObject(Transform go, int idx)
        {
            ...
            {
                if(allowSendIndex)
                    go.SendMessage("Load", new object[]{ datas[idx], idx});
                else
                    go.SendMessage("Load", datas[idx]);
            }
            else
            {
                go.SendMessage("Load", idx);
            }
        ...
        public void RemoveCells()
        {
            if (Application.isPlaying)
            {
                ...
                //ResourceManager.Instance.ClearObjectInUse(prefabName);
                ResourceManager.Instance.ClearPool(prefabName);
            }
        }
        ...
        public virtual void OnScroll(PointerEventData data)
        {
            ...
            position += delta * m_ScrollSensitivity * 27;
        ...
        {
            ResourceManager.Instance.ClearPool(prefabName + "Pool");
        }
    }
}